...
d.aberiae.coords.pres = PRESPOINTS$d.aberiae %>%
  dplyr::select(longitude, latitude)

d.aberiae.coords.back = BACKPOINTS$d.aberiae %>%
  dplyr::select(longitude, latitude)

d.aberiae.coords = rbind(d.aberiae.coords.pres, d.aberiae.coords.back)
d.aberiae.all_resp = c(rep(1, nrow(d.aberiae.coords.pres)), rep(0, nrow(d.aberiae.coords.back)))

d.aberiae.preds = terra::rast("data/d.aberiae_current.tif")
#terra::plot(d.aberiae.preds)
...
myBiomodData <- BIOMOD_FormatingData(
  ...
  resp.var = as.numeric(d.aberiae.vector),
  # Ensure this is a SpatRaster (from terra)
  expl.var = d.aberiae.preds,
  # Force to a basic data.frame
  resp.xy = as.data.frame(d.aberiae.coords), 
  resp.name = "D_aberiae",
  PA.nb.rep = 0,
  filter.raster = TRUE
)
...
writexl::write_xlsx(eval_final, "figures/d.aberiae_model_metrics.xlsx")
...
ggsave("figures/d.aberiae_biomod_performance_box.png", plot = biomod_performance_box, 
...
ggsave("figures/d.aberiae_var_contrib.png", plot = var_importance.plot, 
...
ggsave("figures/d.aberiae_resp_plot.png", plot = resp.plot, 
...
for (yr in years) {
  ...
  # 1. Read raster
  raster_file = paste0("data/d.aberiae_", yr, ".tif")
...
pts <- terra::vect(d.aberiae.coords.pres, geom=c("longitude", "latitude"), crs="EPSG:4326")
...
map.curr +
  geom_point(data = d.aberiae.coords.pres, aes(x = longitude, y = latitude),
...
ggsave("figures/d.aberiae_combomaps.svg", plot = combomaps, 
       width = 8, height = 8, dpi = 450)
ggsave("figures/d.aberiae_combomaps.png", plot = combomaps, 
       width = 8, height = 8, dpi = 450)
...
ggsave("figures/d.aberiae_density_plot.png", plot = density.plot, 
       width = 8, height = 5, dpi = 450)
...
write.csv(stats_summary, "figures/d.aberiae_area_change.csv", row.names = F)


# another way

current_prob.nvir = terra::rast("d.aberiae/proj_Proj_current/proj_Proj_current_d.aberiae_ensemble.tif")
current_prob.nvir <- current_prob.nvir / 1000
current_prob.nvir_masked <- mask(current_prob.nvir, africa_vect)

prob.nvir.2100 = terra::rast("d.aberiae/proj_Proj_2100/proj_Proj_2100_d.aberiae_ensemble.tif")
