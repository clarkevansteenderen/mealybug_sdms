...
p.calceolariae.coords.pres = PRESPOINTS$p.calceolariae %>%
  dplyr::select(longitude, latitude)

p.calceolariae.coords.back = BACKPOINTS$p.calceolariae %>%
  dplyr::select(longitude, latitude)

p.calceolariae.coords = rbind(p.calceolariae.coords.pres, p.calceolariae.coords.back)
p.calceolariae.all_resp = c(rep(1, nrow(p.calceolariae.coords.pres)), rep(0, nrow(p.calceolariae.coords.back)))

p.calceolariae.preds = terra::rast("data/p.calceolariae_current.tif")
#terra::plot(p.calceolariae.preds)
...
myBiomodData <- BIOMOD_FormatingData(
  ...
  resp.var = as.numeric(p.calceolariae.vector),
  # Ensure this is a SpatRaster (from terra)
  expl.var = p.calceolariae.preds,
  # Force to a basic data.frame
  resp.xy = as.data.frame(p.calceolariae.coords), 
  resp.name = "P_calceolariae",
  PA.nb.rep = 0,
  filter.raster = TRUE
)
...
writexl::write_xlsx(eval_final, "figures/p.calceolariae_model_metrics.xlsx")
...
ggsave("figures/p.calceolariae_biomod_performance_box.png", plot = biomod_performance_box, 
...
ggsave("figures/p.calceolariae_var_contrib.png", plot = var_importance.plot, 
...
ggsave("figures/p.calceolariae_resp_plot.png", plot = resp.plot, 
...
for (yr in years) {
  ...
  # Set CRS and Names
  crs(r) = "EPSG:4326"
  names(r) = names(p.calceolariae.preds)
...
pts <- terra::vect(p.calceolariae.coords.pres, geom=c("longitude", "latitude"), crs="EPSG:4326")
...
map.curr +
  geom_point(data = p.calceolariae.coords.pres, aes(x = longitude, y = latitude),
...
ggsave("figures/p.calceolariae_combomaps.svg", plot = combomaps, 
       width = 8, height = 8, dpi = 450)
ggsave("figures/p.calceolariae_combomaps.png", plot = combomaps, 
       width = 8, height = 8, dpi = 450)
...
ggsave("figures/p.calceolariae_density_plot.png", plot = density.plot, 
       width = 8, height = 5, dpi = 450)
...
write.csv(stats_summary, "figures/p.calceolariae_area_change.csv", row.names = F)


# another way

current_prob.nvir = terra::rast("p.calceolariae/proj_Proj_current/proj_Proj_current_p.calceolariae_ensemble.tif")
current_prob.nvir <- current_prob.nvir / 1000
current_prob.nvir_masked <- mask(current_prob.nvir, africa_vect)

prob.nvir.2100 = terra::rast("p.calceolariae/proj_Proj_2100/proj_Proj_2100_p.calceolariae_ensemble.tif")
